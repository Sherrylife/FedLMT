...
class PositionalEmbedding(nn.Module):
    def __init__(self, embedding_size, cfg):
        super().__init__()
...
class TransformerEmbedding(nn.Module):
    def __init__(self, num_tokens, embedding_size, dropout, rate, cfg):
        super().__init__()
        self.num_tokens = num_tokens
        self.embedding_size = embedding_size
        self.positional_embedding = PositionalEmbedding(embedding_size, cfg)
...
class Transformer(nn.Module):
    def __init__(self, num_tokens, embedding_size, num_heads, hidden_size, num_layers, dropout, rate, cfg):
        super().__init__()
        self.num_tokens = num_tokens
    ...
    def forward(self, input):
        ...
        src = input.clone()
        src = src.unsqueeze(0) # the first dimension is the batch size
        N, S = src.size()
...
def transformer(model_rate=1, cfg=None):
    ...
    model = Transformer(num_tokens, embedding_size, num_heads, hidden_size, num_layers, dropout, scaler_rate, cfg)
